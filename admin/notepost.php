...
if (isset($_GET['deletenote'])) {
    ...
    if (mysqli_query($con, $deleteQuery)) {
        ...
        header("Location: " . $_SERVER['PHP_SELF'] . '?success=note deleted successfully');
        exit(); // Make sure to exit after the redirect
    } else {
        header("Location: " . $_SERVER['PHP_SELF'] . '?success=can not delete note');
    }
...
<body>
    ...
    <script>
        ...
        const deleteBtnClk = (id) => {
            ...
            deleteLink.setAttribute("href", `./notepost.php?deletenote=${id}`)
        }
